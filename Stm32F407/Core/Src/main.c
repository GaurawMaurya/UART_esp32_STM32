...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  uint8_t pDataReceived[1024];  			// Buffer to store received data
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  * 			example sending 10 bites at 9600 baud = 10*10/9600 = (approx)10.4 ms
	  */
	 ...
	 HAL_Delay(200);
	 HAL_StatusTypeDef status = HAL_UART_Receive(&huart2, pDataReceived, 1, 500);
	 if(status ==  HAL_OK){
		uint8_t received_length = strlen((char *)pDataReceived);
	  	pDataReceived[received_length] = '\0';
	  	printf("Data Received= %s\n", pDataReceived);
	  	HAL_UART_Transmit(&huart2, (const uint8_t *)"Data Received", 14, 100);
	 }
